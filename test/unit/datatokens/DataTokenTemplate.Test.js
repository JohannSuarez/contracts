...
contract("ERC721Template", (accounts) => {
  ...
  beforeEach("init contracts for each test", async () => {
    ...
    templateERC20 = await ERC20Template.new(
      "TemplateERC20",
      "TEMPLATE20",
      user2,
      web3.utils.toWei("22"),
    ...
    factoryERC20 = await ERC20Factory.new(
      templateERC20.address,
      communityFeeCollector
    );
    templateERC721 = await ERC721Template.new(
      "TemplateERC721",
      "TEMPLATE721",
      admin,
      metadata.address,
    ...
    const trxReceipt = await factoryERC721.createERC721Token(
      "DT1",
      "DTSYMBOL",
      admin,
      metadata.address,
    ...
    const TokenCreatedEventArgs = testUtils.getEventArgsFromTx(
      trxReceipt,
      "TokenCreated"
    );
  ...
  it("should check that the token contract is initialized", async () => {
    assert((await token.isInitialized()) == true, "not initialized");
  });
  ...
  it("should not be able to call create directly in Metadata", async () => {
    await expectRevert(
      metadata.create(tokenAddress, blob, blob, { from: admin }),
      "Metadata: Invalid DataToken Minter"
    );
  });

  it("should update the metadata", async () => {
    await token.updateMetadata(blob, blob, { from: admin });
  });

  it("should not be able to call update directly in Metadata", async () => {
    await expectRevert(
      metadata.update(tokenAddress, blob, blob, { from: admin }),
      "Metadata: Invalid DataToken Minter"
    );
  });
  ...
  it("should not be allowed to update the metadata if not METADATA_ROLE", async () => {
    await expectRevert(
      token.updateMetadata(blob, blob, { from: user2 }),
      "NOT METADATA_ROLE"
    );
  });
  ...
  it("should mint new ERC20Tokens from minter", async () => {
    const trxReceiptERC20 = await token.createERC20(
      blob,
      "ERC20DT1",
      "ERC20DT1Symbol",
      web3.utils.toWei("10"),
    ...
    const ERC20CreatedEventArgs = testUtils.getEventArgsFromTx(
      trxReceiptERC20,
      "ERC20Created"
    );
  ...
  it("should not allow to mint new ERC20Tokens if not ERC721 minter", async () => {
    ...
    await expectRevert(
      erc20Token.mint(user2, web3.utils.toWei("1"), { from: user2 }),
  ...
  it("should add a new template address", async () => {
    await factoryERC20.addTokenTemplate(user2);
    assert((await factoryERC20.getTokenTemplateAddress(1)) == user2);
  });

  it("should get template index by passing address", async () => {
    await factoryERC20.addTokenTemplate(user2);

    assert((await factoryERC20.getTokenTemplateIndex(user2)) == 1);
  });

  it("should throw 999999 if template address doesn't exist", async () => {
    assert((await factoryERC20.getTokenTemplateIndex(reciever)) == 999999);
  });
});
